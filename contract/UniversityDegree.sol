...
// Import necessary library for ERC721 implementation
contract UniversityDegree is ERC721URIStorage {
    ...
    // Function to issue a degree to an address
    ...
    function requestDegree(string memory tokenURI) public {
        ...
        // Update mappings
        personToDegree[msg.sender] = Degree(
            newItemId,
            msg.sender,
            tokenURI,
            false
    ...
    // Function to get not claimed degrees
    function getNotClaimedDegrees() external view returns (Degree[] memory) {
        uint256 count = _tokenIds.current();
        uint256 notClaimedCount = 0;

        // Count the number of not claimed degrees
        ...
        Degree[] memory notClaimedDegrees = new Degree[](notClaimedCount);

        // Populate the result array with not claimed degrees
        uint256 notClaimedIndex = 0;
        for (uint256 i = 1; i <= count; i++) {
            address person = tokenIdToPerson[i];
            if (person != address(0) && !personToDegree[person].issued) {
                notClaimedDegrees[notClaimedIndex] = personToDegree[person];
                notClaimedIndex++;
            }
        }

        return notClaimedDegrees;
    }

    // Function to get all claimed degrees
    function getAllClaimedDegrees() external view returns (Degree[] memory) {
        uint256 count = _tokenIds.current();
        ...
        // Allocate memory for the result array
        Degree[] memory claimedDegrees = new Degree[](claimedCount);

        // Populate the result array with claimed degrees
